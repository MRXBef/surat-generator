...
const handleSaveToDb = async() => {
  try {
    const response = await axiosJWT.post(
      `${import.meta.env.VITE_BASEURL}/surat/domisiliusaha`,
      {
      data_usaha: `${nomorSuratIndex},${namaUsaha},${alamatUsaha},${namaPemilik},${alamatPemilik}`,
...
//ENDTOKEN HANDLER
  ...
  const createPdf = () => {
    ...
    const pdfContent = (
        ...
        <Page style={{padding: '50px'}}>
          ...
          <Text style={StyleSheet.create({textAlign: 'center', textDecoration: 'underline', fontSize: 15, marginTop: 10})}>SURAT KETERANGAN DOMISILI USAHA</Text>
          ...
          <View style={StyleSheet.create({marginTop: 20, display: 'flex', flexDirection: 'row', marginLeft: 80, height: 25})}>
            <Text style={StyleSheet.create({fontSize: 12})}>Nama Usaha</Text>
            <View style={StyleSheet.create({marginLeft: 30, fontSize: 12, position: 'absolute', width: 200, left: 80, display: 'flex', flexDirection: 'row'})}>
                <Text>:</Text>
                <Text style={StyleSheet.create({marginLeft: 5})}>"{namaUsaha.toUpperCase()}"</Text>
            </View>
          </View>
          <View style={StyleSheet.create({marginTop: 20, display: 'flex', flexDirection: 'row', marginLeft: 80, height: 25})}>
            <Text style={StyleSheet.create({fontSize: 12})}>Alamat Usaha</Text>
            <View style={StyleSheet.create({marginLeft: 30, fontSize: 12, position: 'absolute', width: 200, left: 80, display: 'flex', flexDirection: 'row'})}>
                <Text>:</Text>
                <Text style={StyleSheet.create({marginLeft: 5})}>{toUpperCaseFirstStr(alamatUsaha)}</Text>
            </View>
          </View>
          <View style={StyleSheet.create({marginTop: 20, display: 'flex', flexDirection: 'row', marginLeft: 80, height: 25})}>
            <Text style={StyleSheet.create({fontSize: 12})}>Nama Pemilik</Text>
            <View style={StyleSheet.create({marginLeft: 30, fontSize: 12, position: 'absolute', width: 200, left: 80, display: 'flex', flexDirection: 'row'})}>
                <Text>:</Text>
                <Text style={StyleSheet.create({marginLeft: 5})}>{toUpperCaseFirstStr(namaPemilik)}</Text>
            </View>
          </View>
          <View style={StyleSheet.create({marginTop: 20, display: 'flex', flexDirection: 'row', marginLeft: 80, height: 25})}>
            <Text style={StyleSheet.create({fontSize: 12})}>Alamat Pemilik</Text>
            <View style={StyleSheet.create({marginLeft: 30, fontSize: 12, position: 'absolute', width: 200, left: 80, display: 'flex', flexDirection: 'row'})}>
                <Text>:</Text>
                <Text style={StyleSheet.create({marginLeft: 5})}>{toUpperCaseFirstStr(alamatPemilik)}</Text>
            </View>
          </View>
          ...
          <View style={StyleSheet.create({display: 'flex', flexDirection: 'row', gap: 5, marginTop: 5})}>
                ...
                <Text style={StyleSheet.create({fontSize: 12})}>Memang benar Usaha yang namanya tersebut diatas Berdomisili di wilayah kami di {toUpperCaseFirstStr(alamatUsaha)} Kecamatan Mataram.</Text>
          </View>
          ...
          <Text style={StyleSheet.create({marginTop: 30, fontSize: 12, textIndent: 50})}>Demikian Surat keterangan Domisili Usaha ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</Text>
  ...
  return (
    <form action="" style={{width: '100%'}} onSubmit={handlePrint}>
      <h1 className='mb-5 mt-5' style={{fontSize: '20px', color: 'white', fontWeight: 'bold', borderRadius: '10px', textAlign: 'center'}}><span><i style={{position: 'absolute', marginTop: '3px', marginLeft: '-30px'}}><CIcon icon={cilNotes}/></i></span> SURAT KETERANGAN DOMISILI USAHA</h1>
      ...
      <div className="data-diri" style={{backgroundColor: 'white', padding: '100px', width: '100%', borderRadius: '10px'}}>
        ...
        <div className="field">
            <div className="control">
                <input className="input is-fullwidth has-background-white has-text-black" type="text" placeholder="" onChange={(e) => {setNamaUsaha(e.target.value)}} required/>
            </div>
            <p className="help has-text-black">Masukkan nama usaha</p>
        </div>
        <label className="label has-text-black">Alamat Usaha</label>
        <div className="field">
            <div className="control">
                <input className="input is-fullwidth has-background-white has-text-black" type="text" placeholder="" onChange={(e) => {setAlamatUsaha(e.target.value)}} required/>
            </div>
            <p className="help has-text-black">Masukkan alamat usaha</p>
        </div>
        <label className="label has-text-black">Nama Pemilik</label>
        <div className="field">
            <div className="control">
        ...
        <label className="label has-text-black">Alamat Pemilik</label>
        <div className="field">
            <div className="control">
                <input className="input is-fullwidth has-background-white has-text-black" type="text" placeholder="" onChange={(e) => {setAlamatPemilik(e.target.value)}} required/>
            </div>
            <p className="help has-text-black">Masukkan alamat pemilik usaha</p>
        </div>
      </div>

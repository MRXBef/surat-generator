...
const KeteranganUsaha = () => {
  ...
  // usaha yang dimiliki dan keperluan
  ...
  const handleSaveToDb = async() => {
    try {
      const response = await axiosJWT.post(
        `${import.meta.env.VITE_BASEURL}/surat/usaha`,
        {
...
//ENDTOKEN HANDLER
  ...
  const createPdf = () => {
    ...
    const pdfContent = (
      <Document>
        ...
        <Text style={StyleSheet.create({textAlign: 'center', textDecoration: 'underline', fontSize: 15, marginTop: 10})}>SURAT KETERANGAN USAHA</Text>
        ...
        <View style={StyleSheet.create({display: 'flex', flexDirection: 'row', gap: 5, marginTop: 5})}>
              ...
              <Text style={StyleSheet.create({fontSize: 12})}>Berdasarkan keterangannya yang bersangkutan memang benar mempunyai Usaha “{(namaUsaha.toUpperCase())}” usaha ini sudah berdiri sejak tahun {tahun}.</Text>
        </View>
  ...
  return (
    <form action="" style={{width: '100%'}} onSubmit={handlePrint}>
      ...
      {/* USAHA YANG DIMILIKI DAN KEPERLUAN */}
      <div className="data-diri" style={{backgroundColor: 'white', padding: '100px', width: '100%', borderRadius: '10px', marginTop: '30px'}}>
        <h1 className='mb-5' style={{fontSize: '20px', color: 'darkorange', fontWeight: 'bold'}}>USAHA YANG DIMILIKI DAN KEPERLUAN</h1>
        ...
        <div className="field">
            ...
            <p className="help has-text-black">Masukkan nama Usaha anda</p>
        </div>
        <label className="label has-text-black">Tahun Berdiri</label>
        <div className="field">
            <div className="control">
                <input className="input is-fullwidth has-background-white has-text-black" type="number" placeholder="" onChange={(e) => {setTahun(e.target.value)}} required/>
            </div>
            <p className="help has-text-black">Contoh: 2020</p>
        </div>

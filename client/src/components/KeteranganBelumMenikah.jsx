...
const handleSaveToDb = async() => {
  try {
    const response = await axiosJWT.post(
      `${import.meta.env.VITE_BASEURL}/surat/belummenikah`,
      {
...
//ENDTOKEN HANDLER
  ...
  const createPdf = () => {
    ...
    const pdfContent = (
        ...
        <Page style={{padding: '50px'}}>
          ...
          <Text style={StyleSheet.create({textAlign: 'center', textDecoration: 'underline', fontSize: 15, marginTop: 10})}>SURAT KETERANGAN BELUM MENIKAH</Text>
          ...
          <View style={StyleSheet.create({display: 'flex', flexDirection: 'row', gap: 5, marginTop: 5})}>
                ...
                <Text style={StyleSheet.create({fontSize: 12})}>Berdasarkan Status Kartu Tanda Penduduk (KTP) yang bersangkutan memang benar BELUM KAWIN/MENIKAH.</Text>
          </View>
  ...
  return (
    <form action="" style={{width: '100%'}} onSubmit={handlePrint}>
      <h1 className='mb-5 mt-5' style={{fontSize: '20px', color: 'white', fontWeight: 'bold', borderRadius: '10px', textAlign: 'center'}}><span><i style={{position: 'absolute', marginTop: '3px', marginLeft: '-30px'}}><CIcon icon={cilNotes}/></i></span> SURAT KETERANGAN BELUM MENIKAH</h1>
